...
describe('date handling', () => {
  ...
  it('undated record (new addition) sorts before an older dated record', () => {
    // Root-cause regression: July 12 video added without Published Date must NOT
    // sink behind July 8 video that has a Published Date set.
    const raw = [
      row({ 'YouTube ID': vid(1), 'Published Date': '7/8/2026',  title: 'Dated Jul 8' }),
      row({ 'YouTube ID': vid(2), 'Published Date': '',           title: 'Undated new row (Jul 12 in sheet)' }),
    ];
    const result = processVideos(raw);
    assert.equal(result[0].id, vid(2), 'Undated new row must sort before old dated record');
    assert.equal(result[1].id, vid(1));
  });
});
