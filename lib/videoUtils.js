...
function parseDate(dateStr, syncedAt) {
  if (dateStr) {
    // GAS getValues() may return real Date objects
  ...
  if (syncedAt) {
    if (syncedAt instanceof Date) return isNaN(syncedAt.getTime()) ? null : syncedAt;
...
export function processVideos(rawVideos) {
  ...
  items.forEach((item, rowIndex) => {
    try {
      ...
      // Dated records use their timestamp; undated records use a far-future base
      // so they sort ABOVE old dated records (new additions to the sheet surface first).
      // rowIndex tiebreaker keeps later-added rows first among undated records.
      // UNDATED_BASE (~2099) exceeds any realistic publishing date.
      const UNDATED_BASE = 4_102_444_800_000;
      const sortKey = publishedAt ? publishedAt.getTime() : UNDATED_BASE + rowIndex;
